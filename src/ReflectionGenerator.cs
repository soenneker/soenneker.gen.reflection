...
public sealed class ReflectionGenerator : IIncrementalGenerator
{
        ...
        ImmutableArray<string> razorCalls)
    {
        try
        {
            ...
            // We no longer emit a generic fallback or duplicate instance methods to avoid reflection and collisions.
        }
        catch (Exception ex)
        {
            spc.ReportDiagnostic(Diagnostic.Create(
    ...
    private static void GenerateTypeInfoFile(SourceProductionContext spc, ITypeSymbol typeSymbol)
    {
        ...
        // Generate extension method for this specific type
        GenerateExtensionMethodForType(sb, typeSymbol);
    ...
    private static void GenerateExtensionMethodsFile(SourceProductionContext spc, HashSet<ITypeSymbol> generatedTypes)
    {
        try
        {
            ...
            foreach (var typeSymbol in generatedTypes)
            {
                ...
                sb.AppendLine($"    public static TypeInfo GetTypeGen(this {typeName} obj)");
                sb.AppendLine($"    {{");
                sb.AppendLine($"        return new TypeInfo(\"{typeSymbol.Name}\", \"{typeSymbol.ToDisplayString()}\", \"{typeSymbol.ToDisplayString()}, {typeSymbol.ContainingAssembly?.Name ?? "Unknown"}\", {typeSymbol.IsValueType.ToString().ToLower()}, {typeSymbol.IsReferenceType.ToString().ToLower()}, {(typeSymbol is INamedTypeSymbol namedType && namedType.IsGenericType).ToString().ToLower()}, {typeSymbol.IsReferenceType.ToString().ToLower()}, Array.Empty<FieldInfo>());");
                sb.AppendLine($"    }}");
                sb.AppendLine();
            }
    ...
    private static void GenerateTypeInfoClass(StringBuilder sb, ITypeSymbol typeSymbol)
    {
        ...
        sb.AppendLine($"    /// </summary>");
        sb.AppendLine($"    public static string AssemblyQualifiedName => \"{typeSymbol.ToDisplayString()}, {typeSymbol.ContainingAssembly.Name}\";");
        sb.AppendLine();
        sb.AppendLine($"    /// <summary>");
    ...
    private static void GenerateFieldInformation(StringBuilder sb, ITypeSymbol typeSymbol)
    {
        ...
        foreach (var field in allInstanceFields)
        {
            ...
            if (field.DeclaredAccessibility == Accessibility.Public)
            {
                ...
                string getter = $"obj => (({declaringType})obj).{field.Name}";
                string setter = field.IsReadOnly ? "null" : $"(obj, value) => (({declaringType})obj).{field.Name} = ({castType})value";
    ...
    private static void GeneratePropertyInformation(StringBuilder sb, ITypeSymbol typeSymbol)
    {
        ...
        foreach (var property in properties)
        {
            ...
            string getter = property.GetMethod != null ? $"obj => (({declaringType})obj).{property.Name}" : "null";
            string setter = property.SetMethod != null ? $"(obj, value) => (({declaringType})obj).{property.Name} = ({castType})value" : "null";
